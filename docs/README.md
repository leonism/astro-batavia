...
2.  **Type Safety**: Use TypeScript interfaces for all data structures and component props.
3.  **Performance**: Heavy processing should happen at build time (in `getStaticPaths` or service calls during build) or be deferred to the client if necessary (though we prefer SSG).
...
4.  **Component**: `BlogCard.astro` receives the post data and renders it.
